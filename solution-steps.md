...
private_key_file=/home/ec2-user/serdar_client.pem # control_node'da pem key in konulacağı path'i gösterin
...
- name: Install docker
  ...
  tasks:
    ...
    - name: remove serdar_nodejs container if exists
      shell: "docker ps -q --filter 'name=serdar_nodejs' | grep -q . && docker stop serdar_nodejs && docker rm -fv serdar_nodejs && docker image rm serdarcw/nodejs || echo 'Not Found'"
    - name: build container image
      docker_image:
