...
private_key_file=/home/ec2-user/Second.pem 
...
```bash
scp -i Second.pem Second.pem ec2-user@<public-ip of ansible_control>:/home/ec2-user
```
...
```bash
$ ansible all -m ping --key-file "~/Second.pem"
```
...
- name: Install docker
  ...
  # yum-utils is a collection of tools and programs for managing yum repositories, installing debug packages, source packages, extended information from repositories and administration.
    ...
    - name: remove maxim_postgre container and maximcw/postgre if exists
      shell: "docker ps -q --filter 'name=maxim_postgre' && docker stop maxim_postgre && docker rm -fv maxim_postgre && docker image rm -f maximcw/postgre || echo 'Not Found'"
...
- name: Install docker
  ...
  tasks:
    ...
    - name: remove maxim_nodejs container if exists
      shell: "docker ps -q --filter 'name=maxim_nodejs' && docker stop maxim_nodejs && docker rm -fv maxim_nodejs && docker image rm maximcw/nodejs || echo 'Not Found'"
    - name: build container image
      docker_image:
        name: maximcw/nodejs
        build:
...
- name: Install docker
  ...
  tasks:
    ...
    - name: remove maxim_react container and maximcw/react image if exists
      shell: "docker ps -q --filter 'name=maxim_react' && docker stop maxim_react && docker rm -fv maxim_react && docker image rm -f maximcw/react || echo 'Not Found'"
    - name: build container image
      docker_image:
        name: maximcw/react
        build:
